...
class TestPreloadLoggerLoader(unittest.TestCase):
    ...
    def test_python(self):
        ...
        g = ('/usr/bin/python2.7', 1234, '/usr/bin/python2.7 /usr/bin/update-manager --no-update')
        h = self.loader.parsePython(g)
        self.assertIsNotNone(h)
        self.assertNotEqual(g, h)
        ...
        g = ('/usr/bin/python', 1234, '/usr/bin/python /usr/share/software-center/piston_generic_helper.py --datadir /usr/share/software-center/ SoftwareCenterAgentAPI exhibits {"lang": "fr", "series": "trusty"}')
        h = self.loader.parsePython(g)
        self.assertIsNotNone(h)
        self.assertNotEqual(g, h)
        self.assertEqual(h[0], 'piston_generic_helper')
        self.assertEqual(h[2], '/usr/share/software-center/piston_generic_helper.py --datadir /usr/share/software-center/ SoftwareCenterAgentAPI exhibits {"lang": "fr", "series": "trusty"}')
    ...
    def test_java(self):
        ...
        g = ('/usr/bin/java', 1234, '/usr/bin/java /usr/bin/jtestapp /path/to/file --param="some value"')
        h = self.loader.parseJava(g)
        self.assertIsNotNone(h)
        self.assertNotEqual(g, h)
        self.assertEqual(h[0], 'jtestapp')
        self.assertEqual(h[2], '/usr/bin/jtestapp /path/to/file --param="some value"')
    ...
    def test_mono(self):
        g = ('mono-sgen', 1234, 'banshee /usr/lib/banshee/Banshee.exe --redirect-log --play-enqueued')
